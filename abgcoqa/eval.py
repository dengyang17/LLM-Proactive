...
def bleu_cal(sen1, tars):
    bleu1 = sentence_bleu(tars, sen1, weights=(1, 0, 0, 0))
    bleu2 = sentence_bleu(tars, sen1, weights=(0, 1, 0, 0))
    bleu3 = sentence_bleu(tars, sen1, weights=(0, 0, 1, 0))
    bleu4 = sentence_bleu(tars, sen1, weights=(0, 0, 0, 1))
    return bleu1, bleu2, bleu3, bleu4

def cqg_eval(references, generates):
    bleu1_sum, bleu2_sum, bleu3_sum, bleu4_sum, count = 0, 0, 0, 0, 0
    for sen, tar in zip(generates, references):
...
def evaluate(output_file, target_file):
    ...
    for target, output in zip(targets, outputs):
        if target['ambiguity'] == 'ambiguous':
            references.append([tokenize(x.lower()) for x in target['answer']])
            ### Zero-shot
    ...
    print('zero_shot CQG: ' + str(cqg_eval(references, zs_gents)))
    print('few_shot CQG: ' + str(cqg_eval(references, fs_gents)))
    print('zero_shot pcot CQG: ' + str(cqg_eval(references, zs_pcot_gents)))
    print('few_shot pcot CQG: ' + str(cqg_eval(references, fs_pcot_gents)))
...
if __name__ == "__main__":
    evaluate('abgcoqa-vicuna-13B.txt', 'abgcoqa-target.txt')
    evaluate('abgcoqa-chatgpt.txt', 'abgcoqa-target.txt')